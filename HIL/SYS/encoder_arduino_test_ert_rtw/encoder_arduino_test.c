 ...
 * C/C++ source code generated on : Tue Dec 19 21:49:37 2023
 *
...
{
  {
     ...
     *  MATLABSystem: '<S3>/MATLAB System1'
     */
    tmp_0 = (uint32_T)encoder_arduino_test_P.Gain1_Gain_j;
    tmp_1 = (uint32_T)rtb_MATLABSystem1_0;
    sMultiWordMul(&tmp_0, 1, &tmp_1, 1, &tmp.chunks[0U], 2);

    /* Gain: '<S3>/Gain3' */
    sMultiWordMul(&encoder_arduino_test_P.Gain3_Gain.chunks[0U], 2, &tmp.chunks
                  [0U], 2, &encoder_arduino_test_B.r.chunks[0U], 4);

    /* DataTypeConversion: '<S3>/Data Type Conversion1' */
    encoder_arduino_test_B.theta0 = sMultiWord2Double
      (&encoder_arduino_test_B.r.chunks[0U], 4, 0) * 1.9721522630525295E-31;
     ...
     *  MATLABSystem: '<S3>/MATLAB System'
     */
    tmp_0 = (uint32_T)encoder_arduino_test_P.Gain_Gain_c;
    tmp_1 = (uint32_T)rtb_MATLABSystem1_0;
    sMultiWordMul(&tmp_0, 1, &tmp_1, 1, &tmp.chunks[0U], 2);

    /* Gain: '<S3>/Gain2' */
    sMultiWordMul(&encoder_arduino_test_P.Gain2_Gain.chunks[0U], 2, &tmp.chunks
                  [0U], 2, &encoder_arduino_test_B.r1.chunks[0U], 4);

    /* DataTypeConversion: '<S3>/Data Type Conversion' */
    encoder_arduino_test_B.theta1 = sMultiWord2Double
      (&encoder_arduino_test_B.r1.chunks[0U], 4, 0) * 1.9721522630525295E-31;
     ...
     *  Sum: '<S2>/Sum2'
     */
    encoder_arduino_test_B.Control_actionu = (((encoder_arduino_test_B.theta0 -
      encoder_arduino_test_P.ref[0]) * encoder_arduino_test_P.Gain1_Gain *
      encoder_arduino_test_P.K[0] + (rtb_theta0_dot -
      encoder_arduino_test_P.ref[1]) * encoder_arduino_test_P.Gain1_Gain *
      encoder_arduino_test_P.K[1]) + (encoder_arduino_test_B.theta1 -
      encoder_arduino_test_P.ref[2]) * encoder_arduino_test_P.Gain1_Gain *
      encoder_arduino_test_P.K[2]) + (rtb_Abs - encoder_arduino_test_P.ref[3]) *
      encoder_arduino_test_P.Gain1_Gain * encoder_arduino_test_P.K[3];
    ...
    /* Gain: '<Root>/Gain' */
    encoder_arduino_test_B.Gain = encoder_arduino_test_P.Gain_Gain_h *
...
void encoder_arduino_test_initialize(void)
{
  ...
  /* External mode info */
  encoder_arduino_test_M->Sizes.checksums[0] = (1097609297U);
  encoder_arduino_test_M->Sizes.checksums[1] = (2845156871U);
  encoder_arduino_test_M->Sizes.checksums[2] = (1672198514U);
  encoder_arduino_test_M->Sizes.checksums[3] = (1097436240U);
